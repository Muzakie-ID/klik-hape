...
@section('content')
    ...
    @php
        $adminWaNumber = config('services.whatsapp.admin_number', '6285166462782');
    @endphp
    ...
    <div x-data="{ modalOpen: false }">
        ...
        <div class="fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-gray-200 p-3 flex gap-3 max-w-md mx-auto shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
            @if($product->stock > 0)
                <!-- Jika Ready -->
                <a href="https://wa.me/{{ $adminWaNumber }}?text=Halo%20Admin,%20saya%20tertarik%20dengan%20{{ urlencode($product->name) }}" target="_blank" class="flex-1 flex items-center justify-center gap-2 bg-green-500 hover:bg-green-600 text-white font-semibold py-3 px-4 rounded-xl transition">
            ...
            @else
                ...
                <a href="https://wa.me/{{ $adminWaNumber }}?text=Halo%20Admin,%20untuk%20{{ urlencode($product->name) }}%20kapan%20ready%20lagi%20ya?" target="_blank" class="flex-1 flex items-center justify-center gap-2 bg-gray-100 hover:bg-gray-200 text-gray-800 font-semibold py-3 px-4 rounded-xl transition border border-gray-300">
